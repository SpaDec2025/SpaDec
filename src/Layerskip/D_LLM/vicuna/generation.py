...
sys.path.append("../")
from model import ModelArgs_Vicuna, Transformer_vicuna
...
class Vicuna:
    ...
    def build(
        ...
        """
        Build a Vicuna instance by initializing and loading a pre-trained model.

        Args:
            qwen_ckpt_dir (str): Path to the directory containing Llama checkpoint files.
        ...
        Returns:
            Qwen: An instance of the Qwen class with the loaded model and tokenizer.
        ...
        model_path = vicuna_ckpt_dir
        index_file = os.path.join(model_path, "pytorch_model.bin.index.json")
        assert  os.path.exists(index_file), f"no pytorch.bin files found in {model_path}"
        ...
        checkpoints = sorted(Path(dynamic_ckpt_dir).glob("*.pth"))
        assert len(checkpoints) > 0, f"no checkpoint files found in {dynamic_ckpt_dir}"
        assert model_parallel_size == len(
            checkpoints
        ), f"Loading a checkpoint for MP={len(checkpoints)} but world size is {model_parallel_size}"
        dynamic_ckpt_path = checkpoints[get_model_parallel_rank()]
        
        with open(model_args_path, "r") as f:
            params = json.loads(f.read())

        model_args: ModelArgs_Vicuna = ModelArgs_Vicuna(**params)
        ...
        model = Transformer_vicuna(model_args)
        ...
        dynamic_checkpoint = torch.load(dynamic_ckpt_path, map_location="cpu")
        state = dynamic_checkpoint["model"]
        model.load_state_dict(state, strict=False)

        for i in range(params["n_layers"]):
            loraq_weight = state[f"layers.{i}.self_attn.lora_q.linear_A.weight"].T @ state[f"layers.{i}.self_attn.lora_q.linear_B.weight"].T
            lorak_weight = state[f"layers.{i}.self_attn.lora_k.linear_A.weight"].T @ state[f"layers.{i}.self_attn.lora_k.linear_B.weight"].T
            lorav_weight = state[f"layers.{i}.self_attn.lora_v.linear_A.weight"].T @ state[f"layers.{i}.self_attn.lora_v.linear_B.weight"].T

            device = model.layers[i].self_attn.q_proj.weight.device

            model.layers[i].self_attn.q_proj.weight.data += loraq_weight.data.T.to(device)
            model.layers[i].self_attn.k_proj.weight.data += lorak_weight.data.T.to(device)
            model.layers[i].self_attn.v_proj.weight.data += lorav_weight.data.T.to(device)

        print(f"Loaded and merged in {time.time() - start_time:.2f} seconds")

        save_path = f"{dynamic_ckpt_dir}/consolidated.00.pth"
        torch.save(model.state_dict(), save_path)
        print(f"Model saved to {save_path}")

        return Vicuna(model, tokenizer)

    def __init__(self, model: Transformer_vicuna, tokenizer: Tokenizer):
        self.model = model
    ...
    def generate(
        ...
        if min_prompt_len == total_len:
            logits, activation = self.model.forward(tokens, prev_pos)
        ...
        return [
            {
                "generation": {
                    "role": "assistant",
                    "content": self.tokenizer.decode(generation_tokens[i]) if not unsafe else UNSAFE_ERROR,
                },
                "tokens": [self.tokenizer.decode(x) for x in generation_tokens[i]],
                "logprobs": generation_logprobs[i] if logprobs else None,
                "actprobs": generation_actprobs[i] if actprobs else None,
